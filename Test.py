...
custom_objects = {'DepthwiseConv2D': OriginalDepthwiseConv2D}  # type: ignore
...
while True:
    ...
    if hands:
        ...
        if aspectRatio > 1:
            k = imgSize / h
            wCal = math.ceil(k * w)
            imgResize = cv2.resize(imgCrop, (wCal, imgSize))  # Resize correctly
    ...
    if key == ord('s'):
        counter += 1
        cv2.imwrite(f'Image_{time.time()}.jpg', imgWhite)
        print(counter)
